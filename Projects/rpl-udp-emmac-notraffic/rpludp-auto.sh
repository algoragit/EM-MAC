    ...
    mkdir $topology
        ...
        do
            ...
            #sed -i s/\#load\#/${PERIODVALUES[i]}/ udp-sender.c
                ...
                sed -i s/\#randomseed\#/$randomseed/ "trafficLoad-0_zolertia.csc"
                simfile="trafficLoad-0_zolertia.csc"

                echo "Starting simulation of "$topology" with 0 traffic load."
                echo "Iteration number "$j
                #make TARGET=cooja nogui=Simulation $simfile
                java -mx512m -jar ../../../contikitsch/tools/cooja/dist/cooja.jar -nogui=$simfile -contiki='../../../contikitsch'


                for filename in "${RESULTFILES[@]}"
                do
                    mv $filename"_results.csv" $topology"/emmac_"$filename"_results_"${PERIODVALUES[i]}"_iteration-"$j".csv"
                done

                mv "packets.dat" $topology"/emmac_0_iteration-"$j".dat"
                mv "COOJA.testlog" $topology"/testlog_emmac_0_iteration-"$j".txt"
            #done 
            #rm udp-sender.c
        done
