...
static void FireCallback (void *ptr); //Fundamental
static void RadioPowered(int a);
static void CheckConvergence(clock_time_t new, clock_time_t old);
static void broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from); //Fundamental
...
static void processElectionVote(linkaddr_t castID, uint8_t castVOTE);
...
IAmSync(void)
{
	// NOTE: (linkaddr_cmp returns non-zero if the addresses are the same, zero if they are different)
	return ( (linkaddr_cmp(&ChannelSyncNode, &linkaddr_node_addr) != 0) && (NodeChannelState != ELECTION) );
}
...
broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from)
{
	...
	linkaddr_t ReportedSyncNode = r.chan_sync;
	...
	if(IAmSync() && !isSelfJustVoted) { // If I'm the SYNC
		...
		} else if(r.chan_mode != ELECTION && !linkaddr_cmp(&ReportedSyncNode, &linkaddr_node_addr) && r.chan_native == channelTX) { // if more than 1 sync per channel
			if(linkaddr_node_addr.u8[0] < ReportedSyncNode.u8[0]){
				linkaddr_copy(&ChannelSyncNode, &ReportedSyncNode);
			}
		}
	} else { // DESYNC Code
		...
		if (linkaddr_cmp(&ReportedSyncNode, &linkaddr_null) == 0) {
...
static void
processElectionVote(linkaddr_t castID, uint8_t castVOTE)
{
	// 	If it is, we make them the SYNC.
	if (castVOTE > HighestElectionRoll) {
		linkaddr_copy(&ChannelSyncNode, &castID); // (dest, src)
...
// This code does the beacon firing (transmision) for SYNC and DESYNC
static void
FireCallback (void *ptr)
{
	...
	b.node_type = NodeType(); // function returns DESYNCNODE or SYNCNODE 1)
	b.chan_mode = NodeChannelState; //6)
	b.chan_sync = ChannelSyncNode;  //3)
	...
	if (NodeChannelState == ELECTION) {
		...
		linkaddr_t RolledDice = linkaddr_null;
		RolledDice.u8[0] = Roll;
		RolledDice.u8[1] = Roll;
		
		b.chan_mode = ELECTION;
		b.chan_sync = linkaddr_node_addr;
		linkaddr_copy(&ChannelSyncNode, &RolledDice); // (dest, src)
		processElectionVote(linkaddr_node_addr, RolledDice.u8[0]); // process my own vote
...
PROCESS_THREAD(example_desync_process, ev, data)
{
	...
	// Set sync node to 0.0 (NULL) Inicialmente la direccion del nodo SYNC se toma como 0:0:0:0:0:0
	ChannelSyncNode = linkaddr_null;
	
	// Set to DESYNC in CONVERGING state to begin
	NodeChannelState = CONVERGING;
	ThisNodeType = DESYNCNODE;
	
	setChannels(linkaddr_node_addr);  // setup TX and RX channel based on node ID (needs to be dynamic later)
	

	printf("Node (%d.%d) on Channels %u/%u\n",  linkaddr_node_addr.u8[0], linkaddr_node_addr.u8[1], channelTX, channelRX);
	printf("Period=%u, RTIMER_SECOND=%u (%u bytes)\n", rtPERIOD, RTIMER_SECOND, sizeof(rtimer_clock_t));

	random_init((linkaddr_node_addr.u8[0]+linkaddr_node_addr.u8[1]));
